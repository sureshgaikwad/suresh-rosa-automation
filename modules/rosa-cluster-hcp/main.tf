...
resource "rhcs_cluster_rosa_hcp" "rosa_hcp_cluster" {
  ...
  availability_zones = length(var.aws_availability_zones) > 0 ? (
    ...
    length(var.aws_subnet_ids) > 0 && !var.subnet_ids_are_computed ? (
      distinct(data.aws_subnet.provided_subnet[*].availability_zone)
      ) : (
      slice(data.aws_availability_zones.available[0].names, 0, min(length(var.aws_subnet_ids), 3))
    )
  )
...
data "aws_subnet" "provided_subnet" {
  count = var.subnet_ids_are_computed ? 0 : length(var.aws_subnet_ids)
